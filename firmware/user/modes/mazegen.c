...
uint16_t ICACHE_FLASH_ATTR getPathToRoot(uint8_t xsol[], uint8_t ysol[],  uint8_t width, uint8_t height, uint8_t xpoint, uint8_t ypoint, Node * nodes, uint8_t mazescalex, uint8_t mazescaley );
...
get_maze_output_t ICACHE_FLASH_ATTR get_maze(uint8_t width, uint8_t height, uint8_t xleft[], uint8_t xright[], uint8_t ybot[], uint8_t ytop[], uint8_t xsol1[], uint8_t ysol1[], float scxcexits[], float scycexits[], uint8_t mazescalex, uint8_t mazescaley);
...
uint16_t ICACHE_FLASH_ATTR getPathToRoot(uint8_t xsol[], uint8_t ysol[],  uint8_t width, uint8_t __attribute__((unused))height, uint8_t xpoint, uint8_t ypoint, Node * nodes,  uint8_t mazescalex, uint8_t mazescaley )
{
	...
	while (true)
	{
		...
		//os_printf("(%d, %d)", x, y);
		if (point == (Node *)point->parent) break;
...
//        working memory or exceeding MAXNUMWALLS
	...
	os_printf("indSolution %d numcells %d ratio %d / 100\n", indSolution, width*height, 100 * width * height / indSolution);
