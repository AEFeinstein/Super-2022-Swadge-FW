...
1. Check out this repository, move to the ```firmware``` folder, and build it. 
    ...
    ```
1. Flash the firmware to an ESP8266. You will need to set two environment variables so ```makefile``` knows where to find the Swadge. Add the two following variables to your ```.bashrc``` file. ```ESP_PORT``` is the ESP8266's serial port, and will be specific to your machine. ```ESP_FLASH_BITRATE``` is how fast the firmware is flashed. 2000000 is a common value, though if it doesn't work, try something slower, like 1500000.
    ```
    $ nano ~/.bashrc
    ...
    ```
   If you're using WSL, [the Windows port ```COM{N}``` maps to the Linux port ```/dev/ttyS{N}```](https://blogs.msdn.microsoft.com/wsl/2017/04/14/serial-support-on-the-windows-subsystem-for-linux/). If you're using a virtual machine, you'll need to forward your COM port to a Linux serial port. You may have to set read and write permissions on the serial port before flashing.
    ```
